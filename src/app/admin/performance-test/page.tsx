...
export default function PerformanceTestPage() {
  ...
  const runPerformanceTest = async () => {
    ...
    try {
      ...
      testResults.push(`第二次加载失败: ${error}`);
    }
    ...
    for (let i = 0; i < 5; i++) {
      try {
        ...
        testResults.push(`第${i + 1}次连续加载失败: ${error}`);
      }
    }
    ...
    if (times.length > 0) {
      ...
      testResults.push(`连续5次加载 - 平均: ${avg.toFixed(2)}ms, 最小: ${min.toFixed(2)}ms, 最大: ${max.toFixed(2)}ms`);
    }
